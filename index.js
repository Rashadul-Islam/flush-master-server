...
var serviceAccount = require("./configs/flush-master-firebase-adminsdk-r5onm-a1b4262eaa.json");
admin.initializeApp({
...
client.connect(err => {
  const serviceCollection = client.db("flush-master").collection("services");
  const bookingsCollection = client.db("flush-master").collection("bookings");
  const adminsCollection = client.db("flush-master").collection("admins");
  const reviewsCollection = client.db("flush-master").collection("reviews");
  //add service
  ...
  app.patch('/update/:id', (req, res) => {
    serviceCollection.updateOne({ _id: ObjectId(req.params.id) },
      {
        $set: { serviceName: req.body.serviceName, description: req.body.description, imageURL: req.body.imageURL }
      })
      .then(result => {
